...
{% macro default__persist_docs(relation, model, for_relation, for_columns) -%}
  {% if for_relation and config.persist_relation_docs() and model.description %}
    {% set query = alter_relation_comment(relation, model.description) %}
    {% do run_query(query) if query is not none %}
  {% endif %}

  {% if for_columns and config.persist_column_docs() and model.columns %}
    {% set query = alter_column_comment(relation, model.columns) %}
    {% do run_query(query) if query is not none %}
  {% endif %}
{% endmacro %}
